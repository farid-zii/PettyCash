                ...
                <div class="card my-4">
                    ...
                    --}}
                        ...
                        <div class="form-group">
                            <a
                                class="btn mt-4 mx-2 me-5 bg-gradient-dark btn-dark btn-default float-sm-end"
                                href="{{ route('cetakpengajuan', ['bulan' => request('bulan')]) }}" target="_blank"
                                style="color: white;">
                                cetak pengajuan
                            </a>
                        </div>
                    ...
                    <form action="/hrd/pengajuan" method="get">

                        <div class=" mx-2 mt-2" style="display: flex;">
                                    ...
                                    @foreach ($pengajuan as $data)
                                        <tr id="">
                                            ...
                                            <td class="">
                                                ...
                                                <p class="text-xs text-secondary mb-0">{{ $data->user->departemen->nama }}
                                                </p>
                                            </td>
                                            <td class="">
                                            ...
                                            <td class="" style="width: 50px">
                                                {{ Str::words($data->keterangan, 2, '....') }}</td>
                                            <td class="text-center">
                                                @if ($data->approve == 'Menunggu')
                                                    <span class="bg-info p-2 fw-bold"
                                                        style="border-radius:10px;color:white">{{ $data->approve }}</span>
                                                @elseif ($data->approve == 'Setuju')
                                                    <button class="btn btn-success font-weight-bold m-auto"
                                                        data-bs-toggle="modal"
                                                        data-bs-target="#data-{{ $data->id }}-cair">Cairkan</i></button>
                                            ...
                                            <button class="bg-success fw-bold text-light p-2">Cairkan</button>
                                                ...
                                                @elseif ($data->approve == 'Dicairkan')
                                                    <span class="bg-success p-2 fw-bold text-light"
                                                        style="border-radius:10px;">{{ $data->approve }}</span>
                                                @elseif ($data->approve == 'Selesai')
                                                    <span class="bg-primary p-2 fw-bold text-light"
                                                        style="border-radius:10px;">{{ $data->approve }}</span>
                                                @else
                                                    <span class="bg-danger p-2 fw-bold text-light"
                                                        style="border-radius:10px;">{{ $data->approve }}</span>
                                                @endif
                                            </td>
            ...
            namaInput.typeahead({
                ...
                templates: {
                    suggestion: function(data) {
                        return '<div class="custom-suggestion" style="z-index: 999; background: #c3bdbd; ; width: 120%;">' +
                            data.nama + '</div>';
                    },
                }
            });
